                                                ...
                                                <div class="row">
                                                    <div class="col-sm-12">
                                                        <div class="field placeholder">
                                                            ...
                                                            <input id="urls-input" list="urls" name="frm[url]" value="<?= $item['url'] ?>">
                                                            <?php if ($pros || $pages): ?>
                                                                <datalist id="urls">
                                                                    <?php if ($pros): ?>
                                                                        <?php foreach ($pros as $i): ?>
                                                                            <option value="index.php?controller=product&action=detail&id=<?= $item['id'] ?>"><?= $item['title'] ?></option>
                                                                        <?php endforeach; ?>
                                                                    <?php endif; ?>
                                                                    <?php if ($pages): ?>
                                                                        <?php foreach ($pages as $i): ?>
                                                                            <option value="index.php?controller=page&action=detail&id=<?= $item['id'] ?>"><?= $item['title'] ?></option>
                                                                        <?php endforeach; ?>
                                                                    <?php endif; ?>
                                                                </datalist>
                                                            <?php endif; ?>
                                                ...
                                                <div class="row">
                                                    <div class="col-sm-12 text-center">
                                                        <?php if (!empty($item['image_path'])): ?>
                                                            <img width="500px" src="../<?= $item['image_path'] ?>"
                                                                 class=""/>
                                                            <button type="button" class="btn btn-danger" id="removeImg" name="removeImg">حذف عکس</button>
                                                            <input id="image_path" type="hidden" value="<?= $item['image_path'] ?>" name="frm[image_path]">
                                                        <?php else: ?>
